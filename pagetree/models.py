...
class Hierarchy(models.Model):
    ...
    def add_section_from_dict(self, d):
        s = Section.add_root(label="Root", slug="", hierarchy=self, dashboard_info="")
...
class Section(MP_Node):
    ...
    dashboard_info = models.CharField(max_length=512)
    ...
    def append_child(self, label, dashboard_info, slug=''):
        if slug == '':
            slug = slugify(label)
        self.save()
        return self.add_child(label=label, slug=slug, dashboard_info=dashboard_info, hierarchy=self.hierarchy)
    ...
    def as_dict(self):
        return dict(
            label=self.label,
            slug=self.slug,
            dashboard_info=self.dashboard_info,
    ...
    def from_dict(self, d):
        ...
        self.dashboard_info = d.get('dashboard_info','')
    ...
    def add_child_section_from_dict(self, d):
        s = self.append_child(d.get('label', ''), d.get('slug', ''), d.get('dashboard_info',''))
        s.save()
        for pb in d.get('pageblocks', []):
            s.add_pageblock_from_dict(pb)
        for c in d.get('children', []):
