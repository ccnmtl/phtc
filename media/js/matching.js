...
jQuery(document).ready(function($){
  ...
  //add event listener
	...
	function display_answer_comparison(){
		...
		for (key in _answer_key){
			jQuery(jQuery('.table-matching')[_index]).children().children()
				.append('<td class="td-answer-comparison">' + _user_answers[jQuery(jQuery('.answer-label')[_index]).html()] + '</td><td class="td-answer-comparison">'+ _answer_key[jQuery(jQuery('.answer-label')[_index]).html()] +'</td>');
			_index += 1;
		}
